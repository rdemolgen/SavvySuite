...
public class SavvySharedHaplotypes
{
	...
	public static void main(String[] args) throws IOException {
		...
		TreeMap<Integer, VariantContext> storedVariants2 = null;
		...
		for (VariantContext context : vcf) {
			...
			if (!(currentChr.equals(contextChr))) {
				...
				if (bamReader2 != null) {
					storedVariants2 = new TreeMap<Integer, VariantContext>();
				...
				if (ac * 4 > an) {
					...
					if (wildCount + varCount > 0) {
						...
						if ((wildCount == 0) || (varCount == 0)) {
							...
							createSignal(wildCount, varCount, storedBases, storedVariants, context, viterbi, negativeMultiplier, currentChr, vcfSampleNames);
						} else if (bamReader2 == null) {
							viterbi.addSignal(new ViterbiSignal(context.getStart(), -6.0));
							System.err.println(currentChr + "\t" + context.getStart() + "\t-1.1");
						}
					}
					wildCount = bases2[baseIndex(context.getAlleles().get(0).toString().charAt(0))];
					varCount = bases2[baseIndex(context.getAlleles().get(1).toString().charAt(0))];
					if ((wildCount + varCount > 0) && ((wildCount == 0) || (varCount == 0))) {
						if (bamReader2 != null) {
							createSignal(wildCount, varCount, storedBases2, storedVariants2, context, viterbi, negativeMultiplier, currentChr, vcfSampleNames);
						}
					...
					if (bamReader2 != null) {
						wildCount = bases[baseIndex(context.getAlleles().get(0).toString().charAt(0))];
						varCount = bases[baseIndex(context.getAlleles().get(1).toString().charAt(0))];
						if ((wildCount + varCount > 0) && ((wildCount == 0) || (varCount == 0))) {
							boolean thisBase = varCount > 0;
							storedBases2.put(context.getStart(), thisBase);
							storedVariants2.put(context.getStart(), context);
						}
					}
				}
			}
	...
	public static void createSignal(int wildCount, int varCount, TreeMap<Integer, Boolean> storedBases, TreeMap<Integer, VariantContext> storedVariants, VariantContext context, Viterbi viterbi, double negativeMultiplier, String currentChr, List<String> vcfSampleNames) {
		boolean thisBase = varCount > 0;
